...
;(async () => {
  ...
  check(/birthday today/i.test(body), 'the birthday card appears after a successful scan')
  ...
  check(!/birthday today/i.test(await page.locator('body').innerText()), 'closing it dismisses it')

  // a member at another branch must not see them
  const [elsewhereBranch] = await (await svc('/rest/v1/branches', {
    method: 'POST',
    headers: { Prefer: 'return=representation' },
    body: JSON.stringify({ name: 'Probe Branch ' + Date.now() }),
  })).json()
  await svc('/rest/v1/profiles?id=eq.' + celebrant.id, {
    method: 'PATCH',
    body: JSON.stringify({ branch_id: elsewhereBranch.id }),
  })
  ...
  await svc('/rest/v1/branches?id=eq.' + elsewhereBranch.id, { method: 'DELETE' })
