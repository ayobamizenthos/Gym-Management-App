  ...
  return (
    <div className="max-w-3xl animate-rise">
      ...
      <ul className="mt-5 flex flex-col gap-2">
        {rows.map(b => (
          <li key={b.id} className={cn('bg-ink-soft p-4', !b.is_active && 'opacity-50')}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <input
                  defaultValue={b.name}
                  onBlur={e => e.target.value !== b.name && patch(b.id, { name: e.target.value })}
                  className="w-full bg-transparent font-display text-2xl uppercase tracking-tightest outline-none focus:text-volt"
                />
                <input
                  defaultValue={b.address ?? ''}
                  placeholder="Address"
                  onBlur={e => patch(b.id, { address: e.target.value })}
                  className="mt-1 w-full bg-transparent text-sm text-ink-mute outline-none focus:text-paper"
                />
              </div>
              <div className="text-right">
                <p className="font-display text-3xl tabular-nums text-volt">{counts[b.id] ?? 0}</p>
                <p className="text-[10px] uppercase tracking-[0.2em] text-ink-mute">members</p>
              </div>
            </div>
            <div className="mt-3 flex items-center gap-3">
              <Link href={'/admin/branches/' + b.id + '/code'} className="btn-ghost h-9 px-3 text-xs">
                <QrCode size={15} /> Entrance code
              </Link>
              <button onClick={() => remove(b)} className="ml-auto text-ink-mute hover:text-alert" aria-label="Remove">
                <Trash2 size={16} />
              </button>
            </div>
          </li>
      ...
      <section className="mt-6">
        ...
        <button onClick={add} className="btn-volt mt-3"><Plus size={17} /> Add branch</button>
      </section>
    </div>
  )
}
