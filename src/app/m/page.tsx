...
export default function MemberHome() {
  ...
  const state = !profile?.expires_at ? 'none' : !active ? 'out' : soon ? 'due' : 'live'
  const accent = { live: 'text-live', due: 'text-due', out: 'text-out', none: 'text-chalk' }[state]
    ...
    <div className="animate-rise">
      ...
      <section className="relative -mx-5 -mt-6 overflow-hidden">
        ...
        {state !== 'none' && (
          ...
          >
            {state === 'live' ? 'Active' : state === 'due' ? 'Expiring' : 'Expired'}
          </span>
        )}
      </section>
      ...
      <Link
        href="/m/referrals"
        className="panel mt-8 block p-5 transition-colors hover:border-edge hover:bg-base-raised"
      >
