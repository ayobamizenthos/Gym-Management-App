...
export function InstallPrompt() {
  ...
  return (
    <div className="animate-rise fixed bottom-[calc(74px+env(safe-area-inset-bottom))] left-3 z-50 max-w-[calc(100vw-1.5rem)] md:bottom-6 md:left-6">
      <div className="flex items-center border border-ink-line bg-ink-soft/95 backdrop-blur-md">
        <button
          type="button"
          onClick={collapsed ? () => setCollapsed(false) : undefined}
          aria-label={collapsed ? 'Show install option' : 'Zenthos Gym'}
          className={cn(
            'grid h-12 w-12 shrink-0 place-items-center font-display text-xl uppercase tracking-tightest text-volt',
            ...
            <div className="flex items-center gap-2 pr-1.5">
              ...
              {!iosHint && (
                <button
                  ...
                  className="flex h-9 shrink-0 items-center bg-volt px-4 text-[13px] font-bold uppercase text-ink"
                >
                  Install
                </button>
              )}
              ...
              <button
                ...
                aria-label="Collapse"
                className="flex h-9 w-9 shrink-0 items-center justify-center text-ink-mute transition-colors hover:text-paper"
              >
