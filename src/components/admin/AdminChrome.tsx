...
export function AdminChrome({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh lg:grid lg:grid-cols-[230px_1fr]">
      <aside className="hidden border-r border-ink-line lg:flex lg:flex-col">
        ...
        <nav className="flex flex-1 flex-col gap-1 p-3">
          ...
          <Link href="/desk" className="mt-2 flex items-center gap-3 border-t border-ink-line px-3 pt-5 text-sm font-semibold uppercase tracking-wide text-ink-mute hover:text-volt">
            <Activity size={17} /> Front desk
          </Link>
        </nav>
        <div className="border-t border-ink-line p-3">
          <p className="px-3 pb-1 text-xs uppercase tracking-[0.2em] text-ink-mute">Signed in</p>
      ...
      <div className="flex min-h-dvh flex-col">
        <header className="flex h-14 items-center justify-between border-b border-ink-line px-4 lg:hidden">
        ...
        <div className="no-scrollbar flex gap-1 overflow-x-auto border-b border-ink-line px-3 py-2 lg:hidden">
          {LINKS.map(l => {
            const active = path === l.href
            return (
              <Link key={l.href} href={l.href}
                className={cn('shrink-0 px-3 py-1.5 text-xs font-semibold uppercase tracking-wide',
                  active ? 'bg-volt text-ink' : 'text-ink-mute')}>
                {l.label}
              </Link>
            )
          })}
