...
export function MemberChrome({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-2xl flex-col">
      ...
      <nav
        aria-label="Main"
        className="fixed inset-x-0 bottom-0 z-40 mx-auto grid max-w-2xl grid-cols-4 border-t border-ink-line bg-ink"
      >
        {TABS.map(tab => {
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? 'text-volt' : 'text-ink-mute hover:text-paper'
              )}
            >
